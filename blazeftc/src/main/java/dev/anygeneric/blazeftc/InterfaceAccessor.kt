...
class InterfaceAccessor(val module: LynxModule) {
	...
	val robotUSBDevice = ArmableUsbDevice::class.java.getDeclaredField("robotUsbDevice").also { it.isAccessible = true }
	...
	fun replaceUsbStreams(streams: Pair<FileInputStream, FileOutputStream>, action: () -> Unit) {
		...
		robotUSBDevice.set(device, replacement)
		action()
	...
	fun getUSBFD() {
		val rud = robotUSBDevice.get(device ) as RobotUsbDevice
	}
	...
	fun module_status(): ModuleStatus {
		...
		else if (port == null)//not just port, the SerialPort of the TTY implementor
